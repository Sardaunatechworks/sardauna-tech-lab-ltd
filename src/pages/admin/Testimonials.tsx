...
export default function AdminTestimonials() {
  ...
  const [formData, setFormData] = useState({ name: '', company: '', message: '', image: '' });
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      if (err.message.includes('index')) {
        onSnapshot(query(collection(db, 'testimonials')), (snap) => setTestimonials(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
      }
    });
    return () => unsubscribe();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await updateDoc(doc(db, 'testimonials', editingId), { ...dataToSave, updatedAt: serverTimestamp() });
        setSuccessMessage('Testimonial updated successfully!');
      } else {
        await addDoc(collection(db, 'testimonials'), { ...dataToSave, createdAt: serverTimestamp() });
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              <th className="p-4 font-semibold text-gray-600">Person</th>
          ...
          <tbody className="divide-y divide-gray-100">
              ...
              <tr key={testimonial.id} className="hover:bg-gray-50">
                <td className="p-4">
                  <div className="flex items-center gap-3">
                    {testimonial.image && <img src={testimonial.image} alt="" className="w-10 h-10 rounded-full object-cover" />}
                    <div>
                ...
                <td className="p-4 text-gray-600 text-sm line-clamp-2">{testimonial.message}</td>
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setDeletingId(testimonial.id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg">
            ...
            {testimonials.length === 0 && (
              <tr>
                <td colSpan={3} className="p-8 text-center text-gray-500">No testimonials found. Add one to get started.</td>
              </tr>
            )}
          </tbody>
        </table>
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input required type="text" value={formData.company} onChange={e => setFormData({...formData, company: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-brand-blue outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Message</label>
                <textarea required rows={4} value={formData.message} onChange={e => setFormData({...formData, message: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-brand-blue outline-none" />
              </div>
              <div>
