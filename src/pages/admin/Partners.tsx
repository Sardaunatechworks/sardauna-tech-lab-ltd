...
export default function AdminPartners() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      if (err.message.includes('index')) {
        onSnapshot(query(collection(db, 'partners')), (snap) => setPartners(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
      }
    });
    return () => unsubscribe();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedFile) {
        logoUrl = await compressImage(selectedFile);
      }

      const dataToSave = { ...formData, logo: logoUrl };

      if (editingId) {
        await updateDoc(doc(db, 'partners', editingId), { ...dataToSave, updatedAt: serverTimestamp() });
        setSuccessMessage('Partner updated successfully!');
      } else {
        await addDoc(collection(db, 'partners'), { ...dataToSave, createdAt: serverTimestamp() });
        setSuccessMessage('New partner added successfully!');
      }
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              <th className="p-4 font-semibold text-gray-600">Partner</th>
          ...
          <tbody className="divide-y divide-gray-100">
              ...
              <tr key={partner.id} className="hover:bg-gray-50">
                <td className="p-4">
                  <div className="flex items-center gap-3">
                    {partner.logo && <img src={partner.logo} alt="" className="w-10 h-10 rounded object-contain bg-gray-50" />}
                    <p className="font-medium text-gray-900">{partner.name}</p>
                  </div>
                </td>
                <td className="p-4 text-gray-600 text-sm line-clamp-2">{partner.description}</td>
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setDeletingId(partner.id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg">
            ...
            {partners.length === 0 && (
              <tr>
                <td colSpan={3} className="p-8 text-center text-gray-500">No partners found. Add one to get started.</td>
              </tr>
            )}
          </tbody>
        </table>
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  {formData.logo && (
                    <img src={formData.logo} alt="Preview" className="w-12 h-12 rounded object-contain border bg-gray-50" referrerPolicy="no-referrer" />
                  )}
                  <div className="flex-1">
                    <label className="flex items-center justify-center w-full px-4 py-2 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-brand-blue hover:bg-blue-50 transition-colors">
                      <div className="flex items-center gap-2 text-gray-600">
                        ...
                        <span className="text-sm font-medium">{isUploading ? 'Uploading...' : 'Upload Logo'}</span>
                      </div>
                ...
                <div className="mt-2 flex items-center gap-2">
                  ...
                  <input type="url" value={formData.logo} onChange={e => setFormData({...formData, logo: e.target.value})} className="flex-1 px-3 py-1 text-sm border rounded focus:ring-2 focus:ring-brand-blue outline-none" placeholder="https://..." />
                </div>
              </div>
      ...
      {deletingId && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
            ...
            <p className="text-gray-600 mb-6">Are you sure you want to delete this partner? This action cannot be undone.</p>
