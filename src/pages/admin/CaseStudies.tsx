...
export default function AdminCaseStudies() {
  ...
  const [formData, setFormData] = useState({ title: '', problem: '', solution: '', technologies: '', outcome: '', image: '' });
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      if (err.message.includes('index')) {
        onSnapshot(query(collection(db, 'caseStudies')), (snap) => setCaseStudies(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
      }
    });
    return () => unsubscribe();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await updateDoc(doc(db, 'caseStudies', editingId), { ...dataToSave, updatedAt: serverTimestamp() });
        setSuccessMessage('Case study updated successfully!');
      } else {
        await addDoc(collection(db, 'caseStudies'), { ...dataToSave, createdAt: serverTimestamp() });
        setSuccessMessage('New case study added successfully!');
      }
      setIsModalOpen(false);
      setFormData({ title: '', problem: '', solution: '', technologies: '', outcome: '', image: '' });
      setEditingId(null);
      setSelectedFile(null);
    } catch (error: any) {
      toast.error(error.message || 'Error saving case study');
    } finally {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Manage Case Studies</h2>
        <button
          onClick={() => {
            setFormData({ title: '', problem: '', solution: '', technologies: '', outcome: '', image: '' });
            setEditingId(null);
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              ...
              <th className="p-4 font-semibold text-gray-600">Problem</th>
          ...
          <tbody className="divide-y divide-gray-100">
              ...
              <tr key={study.id} className="hover:bg-gray-50">
                <td className="p-4">
                  <div className="flex items-center gap-3">
                    {study.image && <img src={study.image} alt="" className="w-10 h-10 rounded object-cover" />}
                    <p className="font-medium text-gray-900">{study.title}</p>
                  </div>
                </td>
                <td className="p-4 text-gray-600 text-sm line-clamp-2">{study.problem}</td>
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setDeletingId(study.id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg">
            ...
            {caseStudies.length === 0 && (
              <tr>
                <td colSpan={3} className="p-8 text-center text-gray-500">No case studies found. Add one to get started.</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold">{editingId ? 'Edit Case Study' : 'Add Case Study'}</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <textarea required rows={3} value={formData.problem} onChange={e => setFormData({...formData, problem: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-brand-blue outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Solution</label>
                  <textarea required rows={3} value={formData.solution} onChange={e => setFormData({...formData, solution: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-brand-blue outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Technologies (comma-separated)</label>
                <input required type="text" value={formData.technologies} onChange={e => setFormData({...formData, technologies: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-brand-blue outline-none" placeholder="React, Node.js, Firebase" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Outcome</label>
                <textarea required rows={2} value={formData.outcome} onChange={e => setFormData({...formData, outcome: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-brand-blue outline-none" />
              </div>
              <div>
              ...
              <button type="submit" disabled={isUploading} className="w-full bg-brand-blue text-white py-2 rounded-lg font-medium hover:bg-brand-blue/90 disabled:opacity-50">
                {editingId ? 'Update Case Study' : 'Save Case Study'}
              </button>
            </form>
          </div>
        </div>
      ...
      {deletingId && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
            ...
            <p className="text-gray-600 mb-6">Are you sure you want to delete this case study? This action cannot be undone.</p>
